...
if __name__ == '__main__':
    ...
    igng = IncrementalGrowingNeuralGas(epsilon=eps, amature=n_mature, alfac1=0.1, alfacN=0.01)
    numData = 0
    global_train_err = []
    global_test_err = []
    num_mature_neurons = []
    ...
    try:
        ...
        for ep in range(epochs):
            ...
            if mature_neurons_ratio <= 0.98 and (actual_mature_neurons < 0.25*train_data.shape[0]):
    ...
    min_distances = torch.transpose(min_distance_from_centroids(mature_torch_neurons, torch.tensor(data_scaled)), dim0=0, dim1=-1).numpy()
    avg_distances_from_K_centroids = torch.transpose(avg_k_distance_from_centroids(mature_torch_neurons, torch.tensor(data_scaled)), dim0=0, dim1=-1).numpy()
    max_distances_from_K_centroids = torch.transpose(max_k_distance_from_centroids(mature_torch_neurons, torch.tensor(data_scaled)), dim0=0, dim1=-1).numpy()
    outlier_K_factor_in_cluster = torch.transpose(outlier_factor_in_cluster(mature_torch_neurons, torch.tensor(data_scaled)), dim0=0, dim1=-1).numpy()
    outliers = pd.DataFrame(data=np.concatenate((min_distances.reshape(-1, 1),
                                                 ...
                                                 labels.reshape(-1, 1)),
                                                axis=1),
                            columns=['min_distances', 'avg_k_distances', 'max_k_distances', 'outlier_K_factor', 'label'])
